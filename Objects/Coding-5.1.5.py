...
def find_net_force(forces):
    ...
    for single in forces:
        single_0 = single.magnitude
        single_1 = single.angle

        total_horizontal +=single_0*cos(single_1)
        total_vertical +=single_0*sin(single_1)
    ResultMagnitude = (total_horizontal ** 2 + total_vertical ** 2) ** 0.5
    magnitude =round(ResultMagnitude, 1)

    ResultAngle = atan2(total_vertical, total_horizontal)
